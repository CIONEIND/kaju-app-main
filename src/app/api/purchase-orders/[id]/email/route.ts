...
export async function POST(
  ...
  try {
    const user = await requirePermission(PERMISSIONS.ORDERS_SEND_EMAIL);
    ...
    const bodyHtml = `
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#0f172a;max-width:720px;margin:0 auto;">
        ...
        <img src="cid:po-image" alt="Pedido #${order.number}" style="max-width:100%;border-radius:8px;" />
      </div>
    `;
    ...
    const { error } = await resend.emails.send({
      ...
      attachments: [
        {
          content: base64Data,
          filename: `pedido-${order.number}.png`,
          contentId: "po-image",
        },
      ],
    });
    ...
    // Save email record
    await prisma.purchaseOrderEmail.create({
      data: {
        purchaseOrderId: id,
        recipients,
        subject: finalSubject,
    ...
    // Upsert each recipient into EmailRecipient
    await Promise.all(
