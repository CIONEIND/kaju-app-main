...
export function SendEmailModal({ order, state }: SendEmailModalProps) {
  ...
  useEffect(() => {
    if (state.isOpen && order) {
      setRecipients([]);
      setInputValue("");
      setSubject(`Pedido #${order.number} — ${order.clientName}`);
      setBody("");
      setError(null);
      setSuccess(false);
  ...
  async function handleSend() {
    ...
    try {
      ...
      if (!blob) throw new Error("Falha ao gerar imagem do pedido.");
      ...
      // Send email
      const sendRes = await fetch(`/api/purchase-orders/${order.id}/email`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ recipients, subject, body, imageBase64 }),
      });
      ...
      <Modal.Backdrop variant="blur">
        <Modal.Container size="lg">
          <Modal.Dialog>
            <Modal.CloseTrigger />
            <Modal.Header>
              <Modal.Heading>Enviar pedido por e-mail</Modal.Heading>
            </Modal.Header>
            <Modal.Body className="space-y-4">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-muted">
                  Pressione <kbd className="rounded border border-border px-1 py-0.5 font-mono text-[10px]">Enter</kbd>{" "}
                  ou <kbd className="rounded border border-border px-1 py-0.5 font-mono text-[10px]">,</kbd> para adicionar cada e-mail.
                </p>
                ...
                {labels.length > 0 && (
                  <div className="mt-2">
                    ...
                    <div className="mt-1 flex flex-wrap gap-1.5">
                      {labels.map((lbl) => {
                        const labelEmails = lbl.members.map((m) => m.recipient.email);
                        const allAdded = labelEmails.length > 0 && labelEmails.every((e) => recipients.includes(e));
                        return (
                          <span
                            key={lbl.id}
                          ...
                          >
                            ...
                            {allAdded ? (
                              <>
                                {lbl.name}
                                <span className="cursor-pointer opacity-60">({lbl.members.length})</span>
                                <button
                                  aria-label={`Remover etiqueta ${lbl.name}`}
                                  className="cursor-pointer ml-0.5 opacity-60 hover:opacity-100"
                                  onClick={() => {
                                    setRecipients((prev) =>
                                      prev.filter((e) => !labelEmails.includes(e)),
                                    );
                                  }}
                                  type="button"
                                >
                              ...
                              >
                                {lbl.name}
                                <span className="opacity-60">({lbl.members.length})</span>
                              </button>
                            )}
                          </span>
                        );
              ...
              {/* Subject */}
              <div>
                <Label className="mb-1.5 block text-sm font-medium" htmlFor="email-subject">
                  Assunto
                </Label>
                <input
                  id="email-subject"
              ...
              {/* Body */}
              <div>
                <Label className="mb-1.5 block text-sm font-medium" htmlFor="email-body">
                  Mensagem{" "}
                ...
                <textarea
                  ...
                  placeholder="Escreva uma mensagem para acompanhar o pedido..."
                  rows={4}
                  value={body}
                />
              </div>
              ...
              <div className="flex items-start gap-2 rounded-md bg-primary/5 px-3 py-2.5 text-xs text-primary">
                ...
                O resumo do pedido será incluído automaticamente como imagem no corpo do e-mail.
              </div>
